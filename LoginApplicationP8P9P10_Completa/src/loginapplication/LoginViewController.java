...
public class LoginViewController implements Initializable {
    ...
    private void handleButtonEntrarAction(ActionEvent event) throws IOException {
        ...
        if (validarUsuario(usuario, contrasena, rutaArchivo)) {
            ...
            UsuarioLogueado.getInstance().setUsername(usuario);
    ...
    private void handleButtonRegistarAction(ActionEvent event) {
        ...
        if (usuario.isEmpty() || contrasena.isEmpty()) {
            ...
            alert.setContentText("Por favor, complete ambos campos para registrarse.");
            alert.showAndWait();
            return;
        }

        // Ruta del archivo donde se guardarán las credenciales
        String rutaArchivo = "src/loginapplication/ficheros/usuarios.txt";
        ...
        // Mostrar alerta de éxito y borrar campos de texto para permitir que el usuario haga login
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        txtUsername.setText("");
        txtPassword.setText("");
        alert.setTitle("Registro exitoso");
        alert.setHeaderText("Nuevo usuario registrado");
        alert.setContentText("El usuario ha sido registrado correctamente.\npor favor ingresa ahora los datos de login y pulsa en Entrar.");
        alert.showAndWait();
    }
